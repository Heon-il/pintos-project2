...
process_fork (const char *name, struct intr_frame *if_ UNUSED) {
	...
	memcpy(&cur->parent_if, if_, sizeof(struct intr_frame)); // 현재 만드는 ?anjwl
	...
	sema_down(&child->fork_sema); // wait until child load
...
__do_fork (void *aux) {
	...
	/* if child process loaded successfully, -> wake up parent in process_fork */
...
process_exec (void *f_name) {
	...
	success = load (file_name, &_if); // file_name'\0' ~  으로 넘어가기 때문에 제대로 load된다.
...
process_exit (void) {
	...
	// Postpone child 종료 : parent가 exit_status를 받아야 하니깐
...
load (const char *file_name, struct intr_frame *if_) {
	...
	/* for race condition file_name copy*/
	...
	char *token, *save_ptr;
	token = strtok_r(fn_copy, " ", &save_ptr); // only_file_name'\0' ~ 	
	...
	/* file load success -> prepare argv, argc */
	while(token != NULL){
		argv[argc] = token;
	...
	/* $Begin push argc, argv in User Stack */
	...
	/* word align */
	while(if_->rsp % ALIGN_BYTE != 0){
		if_->rsp --;
		*(uint8_t *)(if_->rsp) = 0;
	}

	/* insert addresses(pointers) of string's addresses and sentinel */
	for(int i= argc; i>=0; i--){
		if_->rsp -= ALIGN_BYTE;

		if(i==argc) // sentinel
			memset(if_->rsp, 0, sizeof(ALIGN_BYTE));
		else
			memcpy(if_->rsp, &argv[i], sizeof(ALIGN_BYTE));
	}	

	/* fake return address */
	if_->rsp -= ALIGN_BYTE;
	memset(if_->rsp, 0, ALIGN_BYTE);

	/* Set register value*/
	if_->R.rdi = argc;
	if_->R.rsi = if_->rsp + ALIGN_BYTE;
